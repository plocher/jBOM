...
Usage in KiCad (Eeschema -> Tools -> Generate BOM):
  Command: python3 /absolute/path/to/kicad_jbom_plugin.py %I -i /path/to/INVENTORY.xlsx -o %O [-m] [-v] [-d] [-f "Reference,Value,LCSC"]
...
def main():
    ...
    p.add_argument('-m', '--manufacturer', action='store_true')
    ...
    if not fields:
        if args.manufacturer:
            out_fields += ['Manufacturer', 'MFGPN']
        out_fields += ['Datasheet', 'SMD']
        if args.verbose:
