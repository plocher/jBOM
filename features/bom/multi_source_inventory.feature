Feature: Multi-Source Inventory
  ...
  Scenario: JLC fabricator filtering with multiple inventory files
    ...
    When I generate a JLC BOM with --jlc fabricator and both inventory files
  ...
  Scenario: Source tracking in verbose mode
    ...
    When I generate a verbose JLC BOM with --jlc fabricator
  ...
  Scenario: Multi-source inventory via API
    ...
    And the "JLC_Basic" inventory
    And the "LocalStock" inventory
    When I use the API to generate JLC BOM with --jlc fabricator and both inventory files
  ...
  Scenario: Handle inventory source conflicts
    ...
    When I generate a BOM with --generic fabricator and conflicting inventory data
