...
# Hardcoded steps removed - now handled by parameterized versions in shared.py per Axiom #16
# These steps were:
# - "the inventory includes Mouser part numbers, pricing, and stock quantities for each component" -> {provider}
# - "the inventory contains 3 candidate parts for the 10K resistor..." -> {count:d} {component} {criteria}
# - "the second run uses cached results, does not generate API errors..." -> {error_source}
# - "the search returns statistics showing queries made and success rate" -> {metric_types}
# - "the inventory includes Mouser data for R1 and reports..." -> {provider} {component} {message} {problem_component}
# - "the search presents multiple part options with prices..." -> {item_type} {detail_types}
