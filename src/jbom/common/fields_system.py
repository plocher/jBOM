...
class FieldPresetRegistry:
    ...
    def parse_fields_argument(
        ...
        for tok in tokens:
            if tok.startswith('+'):
                ...
                if preset_name not in self._presets:
                    ...
                    raise ValueError(
                        f\"Unknown preset: {tok} (valid: {valid})\"
                    )
        ...
        return deduped or self.parse_fields_argument(None, available_fields, default_preset)
...
def parse_fields_argument(
    ...
    for name, preset_def in presets.items():
        registry.register_preset(
            name,
            preset_def.get('fields'),
            preset_def.get('description', '')
        )
    
    return registry.parse_fields_argument(
        fields_arg,
        available_fields,
        default_preset
    )
