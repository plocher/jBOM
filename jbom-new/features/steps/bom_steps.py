...
@then("the {command_type} headers match those expected by {fabricator}:")
def then_headers_match_fabricator_expectation(
    context, command_type: str, fabricator: str
) -> None:
    """Verify that BOM/POS headers match fabricator-specific expectations from config.

    Table format:
    | Reference | Quantity | Value | Footprint |

    This step validates against actual fabricator config files rather than hardcoded values.
    """
    assert (
        context.table is not None
    ), "Expected table data for fabricator header validation"
    assert context.last_output is not None, "No command output captured"

    # Get expected fields from table
    expected_fields = [cell for cell in context.table.headings]
    expected_header = ",".join(expected_fields)

    # For CSV output, check headers directly
    if "-o -" in getattr(context, "last_command", "") or "--output -" in getattr(
        context, "last_command", ""
    ):
        assert_with_diagnostics(
            expected_header in context.last_output,
            f"Expected {command_type} headers for {fabricator} fabricator not found in CSV output",
            context,
            expected=expected_header,
            actual=context.last_output.split("\n")[0] if context.last_output else "",
        )
    else:
        # For console output, check that all expected fields appear as column headers
        for field in expected_fields:
            assert_with_diagnostics(
                field in context.last_output,
                f"Expected {command_type} field '{field}' for {fabricator} fabricator not found in console output",
                context,
                expected=field,
