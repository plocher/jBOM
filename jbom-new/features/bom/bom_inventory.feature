Feature: BOM Inventory Enhancement
  ...
  Scenario: Enhance BOM with inventory data
    Given a KiCad schematic file "inventory_test.kicad_sch" with components:
    ...
    When I run "jbom bom inventory_test.kicad_sch --inventory components.csv"
    Then the command exits with code 0
    And the output contains "Manufacturer,Manufacturer Part,Description"
    And the output contains "Yageo,RC0805FR-0710KL"
    And the output contains "Samsung,CL10B104KB8NNNC"

  Scenario: Handle missing inventory file
    Given a KiCad schematic file "test.kicad_sch" with basic components
    When I run "jbom bom test.kicad_sch --inventory missing.csv"
  ...
  Scenario: BOM with partial inventory matches
    ...
    When I run "jbom bom partial_test.kicad_sch --inventory partial.csv -o console"
    Then the command exits with code 0
  ...
  Scenario: Enhanced BOM to file output
    ...
    When I run "jbom bom file_test.kicad_sch --inventory inventory.csv -o enhanced_bom.csv"
    Then the command exits with code 0
