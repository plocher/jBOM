Feature: Legacy and mixed project discovery
  ...
  Scenario: Prefer modern .kicad_pro over legacy .pro when both exist
    Given I am in directory "features/fixtures/kicad_samples/legacy_pro"
    And I create file "features/fixtures/kicad_samples/legacy_pro/legacy.kicad_pro" with content "(kicad_project (version 1))"
  ...
  Scenario: Fall back to legacy .pro when .kicad_pro missing
    When I run jbom command "bom features/fixtures/kicad_samples/legacy_pro -o console -v"
    Then the command should succeed
