Feature: UX messaging and verbosity controls
  ...
  Scenario: Verbose mode reports action and success
    When I run jbom command "bom features/fixtures/kicad_samples/flat_project -o console -v"
  ...
  Scenario: Quiet mode suppresses remediation messages
    When I run jbom command "bom features/fixtures/kicad_samples/flat_project -o console -q"
    Then the command should succeed
    And the error output should be empty

  Scenario: Helpful failure suggestions for empty directory
    Given an empty directory "features/fixtures/empty_dir2"
    When I run jbom command "bom features/fixtures/empty_dir2 -o console"
